...
Qt = QtCore.Qt
settings = QtCore.QSettings('data/settings.ini', QtCore.QSettings.IniFormat)

ghs_path = settings.value("GHS_Path", "")
rpx_path = settings.value("RPX_Path", "")

# Change the following (use / instead of \)
GHS_PATH = os.environ.get("GHS_ROOT", ghs_path)
wiiurpxtool = os.environ.get("CLPC_WIIURPXTOOL_PATH", rpx_path)
...
def main():
    ...
    if len(sys.argv) > 1:
       file_path = sys.argv[1]
        ...
        file_path = settings.value("Project_Path", "")
